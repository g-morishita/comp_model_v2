"""Tests for the sticky social demonstrator-only mixture Stan adapter."""
...
def _social_subject() -> SubjectData:
    ...
    return SubjectData(
        subject_id="social-demo-mixture-sticky-s1",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_demo_mixture_adapter_program_paths_exist() -> None:
    """Ensure the sticky demo-mixture adapter resolves all Stan programs."""

    adapter = SocialRlDemoMixtureStickyStanAdapter()
...
def test_sticky_demo_mixture_adapter_builds_subject_stan_data() -> None:
    """Ensure the sticky demo-mixture adapter exports the expected Stan data."""
...
def test_sticky_demo_mixture_adapter_adds_condition_data() -> None:
    """Ensure the sticky demo-mixture adapter augments conditioned Stan data."""

    subject = SubjectData(
        subject_id="social-demo-mixture-sticky-cond",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_demo_mixture_adapter_subject_param_names() -> None:
    """Ensure the sticky demo-mixture adapter reports correct subject parameters."""
