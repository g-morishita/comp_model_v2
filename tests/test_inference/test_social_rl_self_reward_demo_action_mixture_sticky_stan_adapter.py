"""Tests for the sticky social self-reward + demo-action mixture Stan adapter."""
...
def _social_trial(
    ...
    """Create a minimal social trial for sticky-adapter tests."""
...
def _social_subject() -> SubjectData:
    """Create a social subject with two trials for sticky-adapter tests."""

    return SubjectData(
        subject_id="social-action-mixture-sticky-s1",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_action_mixture_adapter_program_paths_exist() -> None:
    """Ensure the sticky action-mixture adapter resolves all Stan programs."""

    adapter = SocialRlSelfRewardDemoActionMixtureStickyStanAdapter()
...
def test_sticky_action_mixture_adapter_builds_subject_stan_data() -> None:
    """Ensure the sticky action-mixture adapter exports the expected Stan data."""
    ...
    assert stan_data["q_init"] == kernel.v_outcome_init
    assert "reset_on_block" in stan_data


def test_sticky_action_mixture_adapter_adds_condition_data() -> None:
    """Ensure the sticky action-mixture adapter augments conditioned Stan data."""

    subject = SubjectData(
        subject_id="social-action-mixture-sticky-cond",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_action_mixture_adapter_subject_param_names() -> None:
    """Ensure the sticky action-mixture adapter reports correct subject parameters."""

    adapter = SocialRlSelfRewardDemoActionMixtureStickyStanAdapter()
    assert adapter.subject_param_names() == (
        "alpha_self",
        "alpha_other_action",
        "w_imitation",
        "beta",
