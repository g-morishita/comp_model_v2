"""Tests for the sticky social demo-action Stan adapter."""
...
def _social_trial(
    ...
    """Create a minimal social trial for sticky demo-action adapter tests."""
...
def _social_subject() -> SubjectData:
    """Create a social subject with two trials for sticky demo-action tests."""

    return SubjectData(
        subject_id="social-demo-action-sticky-s1",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_demo_action_adapter_program_paths_exist() -> None:
    """Ensure the sticky demo-action adapter resolves all Stan programs."""

    adapter = SocialRlDemoActionStickyStanAdapter()
...
def test_sticky_demo_action_adapter_builds_subject_stan_data() -> None:
    """Ensure the sticky demo-action adapter exports the expected Stan data."""
...
def test_sticky_demo_action_adapter_adds_condition_data() -> None:
    """Ensure the sticky demo-action adapter augments conditioned Stan data."""

    subject = SubjectData(
        subject_id="social-demo-action-sticky-cond",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_demo_action_adapter_subject_param_names() -> None:
    """Ensure the sticky demo-action adapter reports correct subject parameters."""

    adapter = SocialRlDemoActionStickyStanAdapter()
    assert adapter.subject_param_names() == ("alpha_other_action", "beta", "stickiness")
