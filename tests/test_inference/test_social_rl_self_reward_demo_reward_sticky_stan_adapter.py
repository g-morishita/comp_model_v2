"""Tests for the sticky social self-reward + demo-reward Stan adapter."""
...
def _social_subject() -> SubjectData:
    ...
    return SubjectData(
        subject_id="social-self-demo-reward-sticky-s1",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_self_reward_demo_reward_adapter_program_paths_exist() -> None:
    """Ensure the sticky self+demo reward adapter resolves all Stan programs."""
...
def test_sticky_self_reward_demo_reward_adapter_builds_subject_stan_data() -> None:
    """Ensure the sticky self+demo reward adapter exports the expected Stan data."""
...
def test_sticky_self_reward_demo_reward_adapter_adds_condition_data() -> None:
    """Ensure the sticky self+demo reward adapter augments conditioned Stan data."""

    subject = SubjectData(
        subject_id="social-self-demo-reward-sticky-cond",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_self_reward_demo_reward_adapter_subject_param_names() -> None:
    """Ensure the sticky self+demo reward adapter reports correct subject parameters."""

    adapter = SocialRlSelfRewardDemoRewardStickyStanAdapter()
    assert adapter.subject_param_names() == ("alpha_self", "alpha_other", "beta", "stickiness")
