"""Tests for the sticky social demo-action bias Stan adapter."""
...
def _social_subject() -> SubjectData:
    ...
    return SubjectData(
        subject_id="social-action-bias-sticky-s1",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_action_bias_adapter_program_paths_exist() -> None:
    """Ensure the sticky action-bias adapter resolves all Stan programs."""
...
def test_sticky_action_bias_adapter_builds_subject_stan_data() -> None:
    """Ensure the sticky action-bias adapter exports the expected Stan data."""
...
def test_sticky_action_bias_adapter_adds_condition_data() -> None:
    """Ensure the sticky action-bias adapter augments conditioned Stan data."""

    subject = SubjectData(
        subject_id="social-action-bias-sticky-cond",
        blocks=(
            Block(
                block_index=0,
                condition="baseline",
...
def test_sticky_action_bias_adapter_subject_param_names() -> None:
    """Ensure the sticky action-bias adapter reports correct subject parameters."""

    adapter = SocialRlDemoActionBiasStickyStanAdapter()
    assert adapter.subject_param_names() == ("demo_bias", "stickiness")
