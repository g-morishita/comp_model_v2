"""Parameter recovery analysis for social demonstrator-reward learning.

Demonstrates MLE-based parameter recovery for the pure social reward-learning
kernel on a bandit task with pre-choice demonstrator observation. Runs
multiple replications with parallel per-subject fitting and reports standard
recovery metrics.

Usage:
    uv run python example/social_rl_demo_reward/mle_recovery.py
"""
...
def main() -> None:
    ...
    config = ParameterRecoveryConfig(
        ...
        n_subjects=20,
        param_dists=(
            FlatParamDist("alpha_other", stats.norm(-1.386, 0.5), scale="unconstrained"),
