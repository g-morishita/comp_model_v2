"""Simulate social demonstrator-reward learning agents with pre-choice
demonstrator observation, then recover parameters with hierarchical Stan
(NUTS) inference.
...
from comp_model.inference.bayes.stan import SocialRlDemoRewardStanAdapter, StanFitConfig
...
# ── 3. Simulate dataset ────────────────────────────────────────────────────
params_per_subject = {
    f"sub_{i:02d}": SocialRlDemoRewardParams(
        alpha_other=float(true_alpha_others[i]), beta=float(true_betas[i])
    )
...
for sid, p in params_per_subject.items():
    print(f"  {sid}: alpha_other={p.alpha_other:.3f}, beta={p.beta:.3f}")
true_mu_ao = float(sigmoid_vec(TRUE_MU_ALPHA_OTHER_Z))
true_mu_b = float(softplus_vec(np.array(TRUE_MU_BETA_Z)))
print(f"Population: mu_alpha_other={true_mu_ao:.3f}, mu_beta={true_mu_b:.3f}")
...
if "alpha_other_pop" in result.posterior_samples:
...
if "alpha_other" in result.posterior_samples:
    alpha_other_all = result.posterior_samples["alpha_other"]
    beta_all = result.posterior_samples["beta"]
    print(f"\n{'Subject':<12} {'True a_o':>8} {'Post a_o':>10} {'True b':>8} {'Post b':>10}")
    print("-" * 52)
    for i in range(N_SUBJECTS):
        sid = f"sub_{i:02d}"
        print(
