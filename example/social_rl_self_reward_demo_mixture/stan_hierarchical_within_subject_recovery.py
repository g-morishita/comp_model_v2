...
Subject parameters are sampled independently per condition from uniform
distributions using SharedDeltaLayout.

Fit with STUDY_SUBJECT_BLOCK_CONDITION hierarchy + SharedDeltaLayout.
...
task = TaskSpec(task_id="recovery_within_subject_mixture", blocks=tuple(block_specs))
...
# Shared (baseline "strong_demo"): uniform on constrained scale.
# Delta ("weak_demo" - "strong_demo"): normal on unconstrained scale to allow
# meaningful but moderate condition-specific shifts.
param_dists = (
...
print(
    f"Running {config.n_replications} rep x {config.n_subjects} subjects "
    f"x {len(task.blocks)} blocks x {N_TRIALS_PER_BLOCK} trials..."
)
