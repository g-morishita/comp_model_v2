"""Simulate mixture social RL agents with post-outcome demonstrator observation,
...
The demonstrator's action and reward are observed after the subject makes their
own choice and receives their own outcome (SOCIAL_POST_OUTCOME_SCHEMA).

Requires: cmdstanpy and a working CmdStan installation.

Usage:
    uv run python example/social_rl_self_reward_demo_mixture/stan_post_outcome.py
"""
...
task = TaskSpec(
    task_id="social_post_outcome_mixture",
    blocks=(
        BlockSpec(
            condition="social",
            n_trials=N_TRIALS,
