...
Model: social_rl_self_reward_demo_action_mixture_sticky
...
task = TaskSpec(
    task_id="social_rl_self_reward_demo_action_mixture_sticky_condition_hierarchical",
    blocks=(
        BlockSpec(
            condition="baseline",
            n_trials=N_TRIALS,
...
param_dists = (
    ...
    HierarchicalParamDist(
        "stickiness",
        mu_prior=stats.norm(loc=1.0, scale=0.5),
        sd_prior=stats.halfnorm(scale=0.25),
    ),
    HierarchicalParamDist(
        "stickiness__delta",
        mu_prior=stats.norm(loc=0.4, scale=0.4),
        sd_prior=stats.halfnorm(scale=0.2),
    ),
)
...
prior_specs = {
    ...
    "sd_stickiness": PriorSpec("normal", {"mu": 0.0, "sigma": 0.5}),
    "stickiness_delta": PriorSpec("normal", {"mu": 0.4, "sigma": 1.0}),
    "sd_stickiness_delta": PriorSpec("normal", {"mu": 0.0, "sigma": 0.5}),
}
...
param_names = ["alpha_self", "alpha_other_action", "w_imitation", "beta", "stickiness"]
print()
print("Condition-aware hierarchical Stan: social_rl_self_reward_demo_action_mixture_sticky")
print(
