...
task = TaskSpec(
    task_id="asocial_rl_asymmetric_flat_population",
    blocks=(
        BlockSpec(
            condition="example",
            n_trials=N_TRIALS,
...
param_dists = (
    FlatParamDist(
        "alpha_pos",
        stats.uniform(loc=0.35, scale=0.4),
    ),
    FlatParamDist(
        "alpha_neg",
        stats.uniform(loc=0.05, scale=0.3),
    ),
    FlatParamDist(
        "beta",
...
print("Flat population sampling + MLE: asocial_rl_asymmetric")
print(header)
print("-" * len(header))
