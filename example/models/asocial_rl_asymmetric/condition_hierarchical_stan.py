...
task = TaskSpec(
    task_id="asocial_rl_asymmetric_condition_hierarchical",
    blocks=(
        BlockSpec(
            condition="baseline",
            n_trials=N_TRIALS,
...
param_dists = (
    ...
    HierarchicalParamDist(
        "alpha_neg",
        mu_prior=stats.norm(loc=-1.734601, scale=0.5),
        sd_prior=stats.halfnorm(scale=0.3),
    ),
    HierarchicalParamDist(
        "alpha_neg__delta",
        mu_prior=stats.norm(loc=0.635989, scale=0.4),
        sd_prior=stats.halfnorm(scale=0.2),
    ),
    HierarchicalParamDist(
        "beta",
...
prior_specs = {
    "alpha_pos": PriorSpec("normal", {"mu": 0.200671, "sigma": 1.0}),
    "sd_alpha_pos": PriorSpec("normal", {"mu": 0.0, "sigma": 0.5}),
    "alpha_pos_delta": PriorSpec("normal", {"mu": 0.646627, "sigma": 1.0}),
    "sd_alpha_pos_delta": PriorSpec("normal", {"mu": 0.0, "sigma": 0.5}),
    "alpha_neg": PriorSpec("normal", {"mu": -1.734601, "sigma": 1.0}),
    "sd_alpha_neg": PriorSpec("normal", {"mu": 0.0, "sigma": 0.5}),
    "alpha_neg_delta": PriorSpec("normal", {"mu": 0.635989, "sigma": 1.0}),
    "sd_alpha_neg_delta": PriorSpec("normal", {"mu": 0.0, "sigma": 0.5}),
...
print()
print("Condition-aware hierarchical Stan: asocial_rl_asymmetric")
print(
