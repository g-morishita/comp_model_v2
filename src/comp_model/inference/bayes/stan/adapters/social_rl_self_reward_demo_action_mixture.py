"""Stan adapter for the social self-reward + demo-action mixture RL kernel."""
...
class SocialRlSelfRewardDemoActionMixtureStanAdapter:
    """Stan adapter for the social self-reward + demo-action mixture RL kernel.

    Connects the ``SocialRlSelfRewardDemoActionMixtureKernel`` to Stan programs
    that support subject_shared, subject_block_condition_hierarchy,
    study_subject_hierarchy, and study_subject_block_condition_hierarchy
    variants.

    The kernel maintains two independent value systems:

    - ``v_outcome``: updated by the subject's own reward (``alpha_self``).
    ...
    def kernel_spec(self) -> ModelKernelSpec:
        ...
        -------
        ModelKernelSpec
            Static kernel metadata for the social self-reward + demo-action
            mixture RL kernel.
        """
        return SocialRlSelfRewardDemoActionMixtureKernel.spec()
    ...
    def build_stan_data(
        ...
        kernel: SocialRlSelfRewardDemoActionMixtureKernel | None = None,
    ) -> dict[str, Any]:
        """Build Stan data for the self-reward + demo-action mixture programs.
        ...
        kernel
            ...
            Defaults to a fresh ``SocialRlSelfRewardDemoActionMixtureKernel()``.
    ...
    def subject_param_names(self) -> tuple[str, ...]:
        ...
        """
        return ("alpha_self", "alpha_other_action", "w_imitation", "beta")
