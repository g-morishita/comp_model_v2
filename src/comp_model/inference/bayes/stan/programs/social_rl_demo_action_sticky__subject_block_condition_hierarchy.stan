 ...
 * Parameters: alpha_other_action[C], beta[C], stickiness[C]
 */
functions {
...
transformed parameters {
  ...
  vector[C] stickiness;
  {
    int d = 0;
    for (c in 1:C) {
      real aao_z = alpha_other_action_shared_z;
      real bz = beta_shared_z;
      real sz = stickiness_shared_z;
      if (c != baseline_cond) {
        d += 1;
        aao_z += alpha_other_action_delta_z[d];
        bz += beta_delta_z[d];
        sz += stickiness_delta_z[d];
      }
      alpha_other_action[c] = inv_logit(aao_z);
      beta[c] = log1p_exp(bz);
      stickiness[c] = sz;
    }
  }
