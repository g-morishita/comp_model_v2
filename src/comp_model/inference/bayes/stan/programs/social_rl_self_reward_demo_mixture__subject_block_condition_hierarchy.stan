 ...
 * Parameters: alpha_self[C], alpha_other_outcome[C], alpha_other_action[C],
 ...
 * Each condition's five parameters are constructed by adding an unconstrained delta to the
...
data {
  ...
  int alpha_other_outcome_prior_family;   // prior family code for the baseline alpha_other_outcome
...
parameters {
  ...
  real alpha_other_outcome_shared_z; // unconstrained baseline demonstrator-outcome learning rate
  ...
  vector[C - 1] alpha_other_outcome_delta_z; // unconstrained condition offsets for alpha_other_outcome
...
transformed parameters {
  ...
  vector<lower=0,upper=1>[C] alpha_other_outcome; // per-condition demonstrator-outcome learning rate in (0,1)
...
model {
  ...
  for (e in 1:E) {
    ...
    if (step_social_action[e] > 0) {
      ...
      int cc = step_condition[e];
      Q[sa] = Q[sa] + alpha_other_outcome[cc] * (step_social_reward[e] - Q[sa]);          // outcome update
      T[sa] = T[sa] + alpha_other_action[cc] * (1 - T[sa]);                                // chosen action tendency toward 1
      for (a in 1:A) if (sa != a) T[a] = T[a] + alpha_other_action[cc] * (0 - T[a]);     // unchosen toward 0
    }
  }
}
  ...
  vector[D] log_lik = rep_vector(0.0, D); // per-decision log-likelihood for LOO-CV
  {
    ...
    for (e in 1:E) {
      ...
      if (step_social_action[e] > 0) {
        ...
        T[sa] = T[sa] + alpha_other_action[cc] * (1 - T[sa]);
        for (a in 1:A) if (sa != a) T[a] = T[a] + alpha_other_action[cc] * (0 - T[a]);
      }
    }
  }
}
