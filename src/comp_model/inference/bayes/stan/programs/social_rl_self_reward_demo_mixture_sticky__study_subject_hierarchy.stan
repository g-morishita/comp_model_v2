 ...
 *
 * Extends the self-reward + demonstrator-mixture hierarchy with an identity-
 * scale stickiness term that adds a logit bonus to the subject's previous own
 * choice.
 */
functions {
...
data {
  ...
  int stickiness_prior_family;            // prior family code for the group-level stickiness mean
  ...
  int sd_stickiness_prior_family;         // prior family code for the group-level stickiness SD
  real sd_stickiness_prior_p1;            // first hyperparameter of the stickiness SD prior
  real sd_stickiness_prior_p2;            // second hyperparameter of the stickiness SD prior
  real sd_stickiness_prior_p3;            // third hyperparameter of the stickiness SD prior
}
parameters {
  ...
  vector[N] raw_alpha_self_z;             // per-subject standard normal deviates for alpha_self
  ...
  vector[N] raw_alpha_other_outcome_z;    // per-subject standard normal deviates for alpha_other_outcome
  ...
  vector[N] raw_alpha_other_action_z;     // per-subject standard normal deviates for alpha_other_action
  ...
  vector[N] raw_w_imitation_z;            // per-subject standard normal deviates for w_imitation
  ...
  vector[N] raw_beta_z;                   // per-subject standard normal deviates for beta

  real mu_stickiness_z;                   // group-level mean of stickiness
  real<lower=0> sd_stickiness_z;          // group-level SD of stickiness
  vector[N] raw_stickiness_z;             // per-subject standard normal deviates for stickiness
}
...
model {
  ...
  target += prior_lpdf(sd_alpha_self_z | sd_alpha_self_prior_family, sd_alpha_self_prior_p1, sd_alpha_self_prior_p2, sd_alpha_self_prior_p3);
  ...
  for (e in 1:E) {
    ...
    if (step_social_action[e] > 0) {
      ...
      T[n] = (1 - alpha_other_action[n]) * T[n];                                         // decay all action tendencies toward 0
      T[n][sa] = T[n][sa] + alpha_other_action[n];                                        // chosen action gets the toward-1 increment
    }
  }
}
