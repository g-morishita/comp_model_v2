 ...
 * Parameters: alpha_self, alpha_other_outcome, alpha_other_action, w_imitation, beta
 *
 * Mixture social Q-learning for a single subject that learns from own outcomes and a
 * demonstrator's outcomes and actions. Two independent value systems are maintained:
 * Q (outcome values) updated by self and demonstrator rewards, and T (action tendencies)
...
model {
  ...
  for (e in 1:E) {
    ...
    // OWN-OUTCOME UPDATE: Rescorla-Wagner update on Q with alpha_self
