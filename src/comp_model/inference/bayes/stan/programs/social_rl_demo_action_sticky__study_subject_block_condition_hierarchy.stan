 ...
 * Parameters: alpha_other_action[N][C], beta[N][C], stickiness[N][C]
 */
functions {
...
transformed parameters {
  vector[N] alpha_other_action_shared_z =
      ...
      + sd_alpha_other_action_shared_z * raw_alpha_other_action_shared_z;
  ...
  for (d in 1:(C - 1)) {
    alpha_other_action_delta_z[d] =
        mu_alpha_other_action_delta_z[d]
        + sd_alpha_other_action_delta_z[d] * raw_alpha_other_action_delta_z[d];
  ...
  for (n in 1:N) {
    int d = 0;
    for (c in 1:C) {
      real aao_z = alpha_other_action_shared_z[n];
      real bz = beta_shared_z[n];
      real sz = stickiness_shared_z[n];
      if (c != baseline_cond) {
        d += 1;
        aao_z += alpha_other_action_delta_z[d][n];
        bz += beta_delta_z[d][n];
        sz += stickiness_delta_z[d][n];
      }
      alpha_other_action[n][c] = inv_logit(aao_z);
...
generated quantities {
...

  {
    int d_idx = 0;
    for (c in 1:C) {
      real aao_z = mu_alpha_other_action_shared_z;
      real bz = mu_beta_shared_z;
      real sz = mu_stickiness_shared_z;
      if (c != baseline_cond) {
        d_idx += 1;
        aao_z += mu_alpha_other_action_delta_z[d_idx];
        bz += mu_beta_delta_z[d_idx];
        sz += mu_stickiness_delta_z[d_idx];
      }
      alpha_other_action_pop[c] = inv_logit(aao_z);
