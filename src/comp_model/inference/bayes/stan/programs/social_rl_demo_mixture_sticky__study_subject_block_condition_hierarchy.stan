...
data {
  ...
  int alpha_other_outcome_delta_prior_family;
  real alpha_other_outcome_delta_prior_p1;
  real alpha_other_outcome_delta_prior_p2;
  real alpha_other_outcome_delta_prior_p3;
  ...
  int sd_alpha_other_outcome_delta_prior_family;
  real sd_alpha_other_outcome_delta_prior_p1;
  real sd_alpha_other_outcome_delta_prior_p2;
  real sd_alpha_other_outcome_delta_prior_p3;
...
parameters {
  real mu_alpha_other_outcome_shared_z;
  real<lower=0> sd_alpha_other_outcome_shared_z;
  ...
  vector[C - 1] mu_alpha_other_outcome_delta_z;
  vector<lower=0>[C - 1] sd_alpha_other_outcome_delta_z;
  ...
  vector[N] raw_alpha_other_outcome_shared_z;
  ...
  array[C - 1] vector[N] raw_alpha_other_outcome_delta_z;
...
transformed parameters {
  vector[N] alpha_other_outcome_shared_z = mu_alpha_other_outcome_shared_z
                                           + sd_alpha_other_outcome_shared_z
                                           * raw_alpha_other_outcome_shared_z;
  vector[N] alpha_other_action_shared_z = mu_alpha_other_action_shared_z
                                          + sd_alpha_other_action_shared_z
                                          * raw_alpha_other_action_shared_z;
  vector[N] w_imitation_shared_z = mu_w_imitation_shared_z
                                   + sd_w_imitation_shared_z * raw_w_imitation_shared_z;
  vector[N] beta_shared_z = mu_beta_shared_z + sd_beta_shared_z * raw_beta_shared_z;
  vector[N] stickiness_shared_z = mu_stickiness_shared_z
                                  + sd_stickiness_shared_z * raw_stickiness_shared_z;
  ...
  for (d in 1:(C - 1)) {
    alpha_other_outcome_delta_z[d] = mu_alpha_other_outcome_delta_z[d]
                                     + sd_alpha_other_outcome_delta_z[d]
                                     * raw_alpha_other_outcome_delta_z[d];
    alpha_other_action_delta_z[d] = mu_alpha_other_action_delta_z[d]
                                    + sd_alpha_other_action_delta_z[d]
                                    * raw_alpha_other_action_delta_z[d];
    w_imitation_delta_z[d] = mu_w_imitation_delta_z[d]
                             + sd_w_imitation_delta_z[d] * raw_w_imitation_delta_z[d];
    beta_delta_z[d] = mu_beta_delta_z[d] + sd_beta_delta_z[d] * raw_beta_delta_z[d];
    stickiness_delta_z[d] = mu_stickiness_delta_z[d]
                            + sd_stickiness_delta_z[d] * raw_stickiness_delta_z[d];
  }
...
model {
  ...
  for (d in 1:(C - 1))
    target += prior_lpdf(mu_alpha_other_outcome_delta_z[d] | alpha_other_outcome_delta_prior_family, alpha_other_outcome_delta_prior_p1, alpha_other_outcome_delta_prior_p2, alpha_other_outcome_delta_prior_p3);
  for (d in 1:(C - 1))
...
generated quantities {
...

  {
    int d_idx = 0;
    for (c in 1:C) {
      real aooz = mu_alpha_other_outcome_shared_z;
      real aoaz = mu_alpha_other_action_shared_z;
      real wiz = mu_w_imitation_shared_z;
      real bz = mu_beta_shared_z;
      real sz = mu_stickiness_shared_z;
      if (c != baseline_cond) {
        d_idx += 1;
        aooz += mu_alpha_other_outcome_delta_z[d_idx];
        aoaz += mu_alpha_other_action_delta_z[d_idx];
        wiz += mu_w_imitation_delta_z[d_idx];
        bz += mu_beta_delta_z[d_idx];
        sz += mu_stickiness_delta_z[d_idx];
      }
      alpha_other_outcome_pop[c] = inv_logit(aooz);
      alpha_other_action_pop[c] = inv_logit(aoaz);
